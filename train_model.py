...
def create_tfrecord_for_prediction(batch_size, stoke_data, tfrecord_file):
    def parse_line(stoke_data):
        ...
        inkarray = json.loads(stoke_data)['drawing']
...
def main(unused_args):
  ...
  if FLAGS.predict_for_data != None:
      json_data = open(FLAGS.predict_for_data).read()
      print("I Starting prediction")
      class_names = get_classes()
      create_tfrecord_for_prediction(FLAGS.batch_size, json_data, FLAGS.predict_temp_file)
      ...
      for idx, prediction in enumerate(predict_results):
          ...
          print(" Predicted Class is: %s with a probability of %f" % (class_name, probability))
...
if __name__ == "__main__":
  ...
  parser.add_argument(
      ...
      type=str,
      default=None,
      help="Path to temp mid data (tf.Example in TFRecord format)")  
